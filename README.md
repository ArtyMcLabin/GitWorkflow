...
a) Git Operations
    ...
    If(neither exists){
       nested if(i explicitly told "don't use GitWorkflow"){
           create .cursor/rules/no_gitworkflow.mdc to remember this decision for the specific project.
    }else{
       a1) if you are not Claude's LLM model - give exactly that warning to me: "GitWorkflow strongly suggests to run the initial setup using Claude. ChatGPT is known to skip steps. as of 25may2025"  and wait for user before you do anything else.
