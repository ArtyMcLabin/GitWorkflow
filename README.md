...
2. Add these rules to your Cursor Settings -> Rules -> User Rules:
   ...
   For any git-related operations: First check for .cursor/rules/ for "use_gitworkflow.mdc" or "no_gitworkflow.mdc" in the project. If neither exists, read https://github.com/ArtyMcLabin/GitWorkflow/README.md and follow its instructions. If explicitly told "don't use GitWorkflow", create .cursor/rules/no_gitworkflow.mdc to remember this decision for the specific project.

   # Rule 2: Issue Creation (Optional)
   For GitHub issue creation: Use GitWorkflow's standardized issue creation process. Run git_workflow.ps1 with -CreateIssue switch and appropriate parameters (-IssueTitle, -IssueBody, -IssueLabels). This ensures consistent issue formatting, proper labeling (ai-generated, etc.), and cross-repository support.
   ```
...
3. Create the project's use_gitworkflow.mdc with these instructions:
   ```markdown
